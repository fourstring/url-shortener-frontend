...
## 工具函数
工具函数位于`src/utils/tests.ts`下。

### function sleep(ms:number);
该函数用于阻塞异步执行代码，时间为`ms`参数指定的毫秒数。
JavaScript中并不存在任何强行阻塞同步代码的方式，因为v8引擎的执行处于一个大的事件循环中，一旦允许存在这样的方法，那么很容易直接阻塞整个事件循环，结果就是页面陷入冻结状态，js引擎无法再去响应处理页面上的各种事件与用户交互。

但我们可以比较方便地阻塞异步代码，查看sleep的源码可以知道它仅仅是返回一个在`ms`后`resolve`的`Promise`，我们知道，`await`语句会在其后的`Promise`resolve之后才接着执行后续代码（等价于将后续代码转换为`await`后的`Promise`的`then`回调，因此使用`await sleep`即可轻易阻塞异步代码的执行。

由于js引擎的性质，它会在目前的所有同步代码执行完成后，才进行异步任务的调度，因此这样的异步阻塞机制不会带来同步阻塞机制使页面停止响应的问题，但代价是它无法用于需要同步阻塞的代码。

### function getByDeepText(text: string);
遍历DOM树，用于在测试中`render`后获取被包裹在深层组件树中的包含`text`文本的DOM节点。
